...
cur=`pwd`
date
dirs="Beginner/fargate/fargateapp Beginner/irsa Advanced/app-mesh Beginner/fargate extra/sampleapp2 extra/eks-cidr2 extra/nodeg2 Intermediate/sampleapp Launch/lb2 Launch/cicd Launch/nodeg Launch/cluster"
for i in $dirs; do
cd $cur
cd ../$i
echo "**** Destroying in $i ****"
rm -rf .terrform* backend.tf
terraform init -no-color -force-copy -migrate-state > /dev/null
